...
def test_openclaw_payload_shape():
    out = get_payload_builder("openclaw").build_pipeline(
        "pid1", "sequence", "completed", "hello",
        channel="feishu", target="t1")
    assert out == [{"tool": "message", "action": "send",
...
def test_generic_payload_chunks_long_message():
    ...
    assert len(out) == 3
    assert out[0] == {"pipeline_id": "pid1", "mode": "sequence",
                      "status": "completed", "message": "x" * 1800,
...
def test_format_done_renders_artifact_variants():
    artifacts = [
        {"type": "file", "label": "GDD", "pattern": "gdd-*.md",
         "path": "/ws/gdd-ab.md", "url": "https://s3/presigned", "exists": True},
        {"type": "file", "label": "本地", "pattern": "x.md",
         "path": "/ws/x.md", "exists": True},

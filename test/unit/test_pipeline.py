...
class TestPipelineDataStructures:
    ...
    def test_pipeline_to_dict_includes_shared_cwd(self):
        ...
        pl = Pipeline(
            pipeline_id="test-1", mode="sequence", steps=[], context={"shared_cwd": "/tmp/ws"}
        )
        d = pl.to_dict()
...
class TestSequenceExecution:
    ...
    async def test_sequence_chains_output_as_context(self, manager, mock_pool):
        ...
        async def fake_prompt(prompt, idle_timeout=300):
            ...
            yield {
                "method": "x",
                "params": {"type": "message.part", "content": f"result{call_count}"},
            }
    ...
    async def test_conversation_stops_on_done(self, manager, mock_pool):
        ...
        conn = _make_mock_conn(
            [
                {
                    "method": "x",
                    "params": {"type": "message.part", "content": "All good. STATUS: DONE"},
                },
                {"_prompt_result": {"result": {"stopReason": "end"}}},
            ]
        )
                ...
                [
                    {
                        ...
                        "params": {
                            "type": "message.part",
                            "content": "Let me ask @qwen about this"
                            if agent == "kiro"
                            else "Sure, STATUS: DONE",
                        },
                    },
                    {"_prompt_result": {"result": {"stopReason": "end"}}},
                ]
...
async def test_failed_lambda_step_skips_local_fallback(tmp_path):
    ...
    pool.get_or_create = AsyncMock(
        side_effect=AssertionError("lambda step must never reach the local pool")
    )

    lambda_pool = Mock()
    lambda_pool.invoke = AsyncMock(
        return_value={
            "status": "error",
            "error": "lambda blew up",
            "output": "",
            "duration": 1.0,
            "session_id": "s",
        }
...
async def test_recovery_resets_all_race_steps(tmp_path):
    ...
    original._store = original._store.__class__(db_path)
    pl = Pipeline(
        pipeline_id="recovery-race",
        mode="race",
        status="running",
        steps=[
            PipelineStep(
                agent="kiro", prompt_template="a", status="completed", result="old winner"
            ),
...
async def test_recovery_fails_conversation_with_terminal_event(tmp_path):
    ...
    original._store = original._store.__class__(db_path)
    pl = Pipeline(
        pipeline_id="recovery-conversation", mode="conversation", status="running", steps=[]
    )
    original._store.save(pl)
...
async def test_recovery_stops_after_retry_limit(tmp_path):
    ...
    original._store = original._store.__class__(db_path)
    pl = Pipeline(
        pipeline_id="recovery-limit",
        mode="sequence",
        status="running",
        retries=2,
        steps=[PipelineStep(agent="kiro", prompt_template="task")],
    )
    original._store.save(pl)
...
def _mgr(tmp_path, webhook_url="", agents_cfg=None):
    ...
    return PipelineManager(
        pool=pool, agents_cfg=cfg, webhook_url=webhook_url, db_path=str(tmp_path / "test.db")
    ), pool
            ...
            with patch.object(manager, "_webhook_start", new_callable=AsyncMock):
                with patch.object(manager, "_webhook", new_callable=AsyncMock):
                    with patch.object(manager, "_webhook_step", new_callable=AsyncMock):
                        with patch("src.pipeline.get_prompt_suffix", return_value=""):
                            with patch(
                                "src.pipeline.transform_notification", side_effect=fake_transform
                            ):
                                await manager._run(pl)

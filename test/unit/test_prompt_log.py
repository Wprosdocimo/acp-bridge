...
class TestRecord:
    ...
    def test_defaults_rendered_and_final_to_template(self, store):
        rid = store.record(
            parent_type="job", parent_id="j1", agent="kiro", mode="acp", template="bare prompt"
        )
        row = store.get(rid)
    ...
    def test_db_failure_returns_empty_string(self, store):
        ...
        store._db.close()
        rid = store.record(
            parent_type="job", parent_id="j-fail", agent="kiro", mode="acp", final="hello"
        )
        assert rid == ""
    ...
    def test_redact_skipped_when_disabled(self, store):
        ...
        rid = store.record(
            parent_type="job", parent_id="j-noredact", agent="kiro", mode="acp", final=secret
        )
        row = store.get(rid)
...
class TestTruncate:
    def test_oversize_clipped_with_marker(self):
        ...
        try:
            ...
            big = "X" * 5000
            rid = s.record(
                parent_type="job", parent_id="j-big", agent="kiro", mode="acp", final=big
            )
            row = s.get(rid)
    ...
    def test_undersize_unchanged(self, store):
        small = "small prompt"
        rid = store.record(
            parent_type="job", parent_id="j-small", agent="kiro", mode="acp", final=small
        )
        row = store.get(rid)
...
class TestQuery:
    ...
    def test_list_by_parent_isolates_other_pipelines(self, store):
        store.record(
            parent_type="pipeline_step", parent_id="pipe-A", agent="kiro", mode="acp", final="A"
        )
        store.record(
            parent_type="pipeline_step", parent_id="pipe-B", agent="kiro", mode="acp", final="B"
        )
    ...
    def test_search_by_agent(self, store):
        store.record(parent_type="job", parent_id="j1", agent="kiro", mode="acp", final="...")
        store.record(parent_type="job", parent_id="j2", agent="claude", mode="acp", final="...")
        store.record(
            parent_type="heartbeat", parent_id="kiro", agent="kiro", mode="acp", final="..."
        )
...
class TestCleanup:
    def test_cleanup_removes_old_records(self, store):
        ...
        rid = store.record(
            parent_type="job", parent_id="j-old", agent="kiro", mode="acp", final="ancient"
        )
        store._db.execute(
            "UPDATE prompt_log SET created_at=? WHERE record_id=?", (time.time() - 7200, rid)
        )
        store._db.commit()
    ...
    def test_cleanup_zero_retention_noop(self, store):
        store.record(parent_type="job", parent_id="j-keep", agent="kiro", mode="acp", final="...")
